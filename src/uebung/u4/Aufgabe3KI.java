...
public class Aufgabe3KI {
	...
	public static int[] smartMove(int x, int y, boolean firstmove, double distanceBefore, double distanceAfter, int dirX, int dirY) {
		...
		// Gilt nur für ersten Zug (boolean firstmove = true)
		...
		if(firstmove == false) {
			// Hat sich der Abstand zum Geld vergrößert?
			...
			if(dirX == 0 && (distanceBefore <= distanceAfter)) {
				dirX = 1;
			} else if (dirX == 0 && (distanceBefore > distanceAfter)) {
				dirX = 0;
			}

			if(dirX == 1 && (distanceBefore <= distanceAfter)) {
				dirX = 0;
			...
			if(dirY == 0 && (distanceBefore <= distanceAfter)) {
				dirY = 1;
			} else if (dirY == 0 && (distanceBefore > distanceAfter)) {
				dirY = 0;
			}

			if(dirY == 1 && (distanceBefore <= distanceAfter)) {
				dirY = 0;
			} else if(dirY == 1 && (distanceBefore > distanceAfter)) {
				dirY = 1;
			}

			// Wenn Gegner schon am Spielfeldrand steht ...
			if(dirX == 0) {
				if(x > 1) {
					x -= 1;
				}
		...
		// Es wird als Array übergeben: Neue X-Koordinate, Y-Koordinate, x-Richtung und y-Richtung
		return new int[] {x, y, dirX, dirY};
	}


	public static void main(String[] args) {
		// Spieler hat am Anfang kein Geld gessammelt und keine Tür erreicht
		...
		if(!enemy.equals(door)) {
			...
			// Neue Spieler-POsition, wenn Gegner auf gleichem Feld wie Tür ist
			int enemyNewX = (int) (Math.random()*9)+2;
			int enemyNewY = (int) (Math.random()*9)+2;
			enemy.x = enemyNewX;
			enemy.y = enemyNewY;
		...
		do {
			...
			// x- und y-Richtung werden bei Aufruf von smartMove() übergeben, um festzuhalten,
			...
			System.out.print("\nWohin wollen Sie gehen?\nDrücken Sie u (rauf), l (links), r (rechts) oder d (runter): " );
			...
			if(player.equals(money)) {
				moneyCollect = true;
				System.out.println("\nSie haben das Geld eingesammelt! Jetzt schnell zur Tür. Husch husch.");
				// Koordinaten werden auf 0 gesetzt, damit das Geld vom Spielplan verschwindet
				money.x = 0;
				money.y = 0;
			}

			// Wenn der Spieler die Tür erreicht
			if(player.equals(door)) {
				// Wenn der Spieler das Geld bereits eingesammelt hat
				if(moneyCollect == true) {
					reachedDoor = true;
					win = true;
					System.out.println("\nSie haben das Geld gesammelt UND die Tür erreicht. Sie sind ein Held. Sie haben gewonnen.");
					break;
					// Wenn der Spieler das Geld noch nicht eingesammelt hat, aber die Tür erreicht ...
				} else {
					System.out.println("\nSie haben die Tür erreicht, aber noch nicht das Geld eingesammelt.\nSo geht das Spiel nicht!");
				}
			}
